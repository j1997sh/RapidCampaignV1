(function(){
 ...
 async function init(){
  ...
  async function load(){
   ...
   const c=await sb.rpc('rapid_campaign_current_admin_context');if(c.error||!c.data?.length){list.innerHTML='<div class="ops-error"><strong>Campaigns could not be loaded.</strong><button onclick="location.reload()">Retry</button></div>';return;}
   const r=await sb.rpc('org_admin_central_campaign_rollouts',{p_org:c.data[0].organisation_id});if(r.error){list.innerHTML=`<div class="ops-error"><strong>Campaigns could not be loaded.</strong><span>${esc(r.error.message)}</span><button onclick="location.reload()">Retry</button></div>`;return;}rows=r.data||[];draw();
  }
