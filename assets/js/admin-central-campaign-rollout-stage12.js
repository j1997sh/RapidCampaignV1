...
async function init(){
 ...
 function metaPreflight(settings){
   const rows=facebookRows(settings),total=rows.length;
   const global=[];
   if(!settings.campaignPattern)global.push('Campaign naming is missing');
   if(!settings.adSetPattern)global.push('Ad set naming is missing');
   if(!settings.adPattern)global.push('Ad naming is missing');
   if(!settings.objective)global.push('Campaign objective is missing');
   if(!settings.cta)global.push('CTA is missing');
   if(!settings.body)global.push('Primary text is missing');
   if(!settings.headline)global.push('Headline is missing');
   if(!settings.pageId)global.push('Facebook Page ID is missing');
   if(!Number(settings.budget)||Number(settings.budget)<=0)global.push('Budget must be greater than zero');
   if(!settings.country)global.push('Country is missing');
   const checked=rows.map(x=>{
     const issues=[];
     if(x.site.status!=='live')issues.push('Site not live');
     if(!x.audienceRef)issues.push('Audience missing');
     if(!x.creative)issues.push('Creative missing');
     if(!x.row.Link)issues.push('Landing URL missing');
     if(!x.row['Campaign Name'])issues.push('Campaign name missing');
     if(!x.row['Ad Set Name'])issues.push('Ad set name missing');
     if(!x.row['Ad Name'])issues.push('Ad name missing');
     return {...x,issues};
   });
   return {rows:checked,total,global,ready:global.length===0&&total>0&&checked.every(x=>x.issues.length===0)};
 }
 function renderFacebookPreflight(){
   const s=facebookSettings(),check=metaPreflight(s),rows=check.rows,total=check.total;
   const live=rows.filter(x=>x.site.status==='live').length,aud=rows.filter(x=>x.audienceRef).length,cre=rows.filter(x=>x.creative).length,links=rows.filter(x=>x.row.Link).length,readyRows=rows.filter(x=>!x.issues.length).length;
   if(downloadFacebookAds){downloadFacebookAds.disabled=!check.ready;downloadFacebookAds.title=check.ready?'Ready for Meta Ads Manager':'Resolve every pre-flight item before final export';}
   const issueText=[...check.global,...rows.flatMap(x=>x.issues.map(i=>`${x.site.area}: ${i}`))];
   facebookAdsPreflight.innerHTML=`<div class="facebook-preflight-kpis"><div><span>Microsites live</span><strong>${live}/${total}</strong></div><div><span>Audiences</span><strong>${aud}/${total}</strong></div><div><span>Creatives</span><strong>${cre}/${total}</strong></div><div><span>Tracking URLs</span><strong>${links}/${total}</strong></div><div class="${check.ready?'ready-ok':''}"><span>Meta ready</span><strong>${readyRows}/${total}</strong></div></div>${check.ready?`<div class="state-banner success"><strong>Ready for Meta upload.</strong> All ${total} local ad rows are complete. The exported campaign, ad sets and ads are set to PAUSED so the upload cannot start spending automatically.</div>`:`<div class="state-banner error"><strong>Final export locked.</strong> ${issueText.length} pre-flight item${issueText.length===1?'':'s'} need attention before the Meta-ready CSV can be downloaded.</div>`}<div class="facebook-preflight-table"><table><thead><tr><th>Area</th><th>Campaign</th><th>Ad set</th><th>Ad</th><th>Audience</th><th>Creative</th><th>Status</th></tr></thead><tbody>${rows.slice(0,12).map(x=>`<tr><td>${esc(x.site.area)}</td><td>${esc(x.row['Campaign Name'])}</td><td>${esc(x.row['Ad Set Name'])}</td><td>${esc(x.row['Ad Name'])}</td><td class="${x.audienceRef?'':'facebook-missing'}">${esc(x.audienceRef||x.expectedAudience+' — missing ID')}</td><td class="${x.creative?'':'facebook-missing'}"><code>${esc(x.creative?.filename||'Missing')}</code></td><td>${x.issues.length?`<span class="facebook-missing">${esc(x.issues.join(', '))}</span>`:'Ready'}</td></tr>`).join('')}</tbody></table>${rows.length>12?`<small>Showing 12 of ${rows.length} ads.</small>`:''}</div>`;
 }
 ...
 facebookAdsForm.onsubmit=async e=>{e.preventDefault();const settings=facebookSettings(),check=metaPreflight(settings);renderFacebookPreflight();if(!check.ready){facebookSettingsMessage.innerHTML='<div class="state-banner error">Meta-ready export is locked until every pre-flight check passes.</div>';return;}await persistFacebookSettings(false);const csv=[META_ADS_HEADERS.map(csvCell).join(','),...check.rows.map(x=>META_ADS_HEADERS.map(h=>csvCell(x.row[h])).join(','))].join('\r\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`${(data.rollout.title||'facebook-campaign').toLowerCase().replace(/[^a-z0-9]+/g,'-')}-meta-ready.csv`;a.click();URL.revokeObjectURL(a.href);msg(`${check.rows.length} Meta-ready ad row${check.rows.length===1?'':'s'} generated.`);facebookAdsDialog.close();};
